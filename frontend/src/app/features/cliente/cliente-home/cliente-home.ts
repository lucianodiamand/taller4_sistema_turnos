...
/** Home del cliente: saludo, próximo turno y accesos directos. */
@Component({
  ...
  template: `
    ...
    <mat-card appearance="outlined" class="proximo">
      ...
      <mat-card-content>
        @if (proximo(); as t) {
          ...
          <p>{{ t.servicio?.nombre }} · {{ t.profesional?.usuario?.nombre }}</p>
        } @else {
    ...
    <div class="accesos">
      <a mat-card routerLink="/inicio/cliente/disponibilidades" class="acceso">
        <mat-icon>event_available</mat-icon><span>Reservar turno</span>
      </a>
      <a mat-card routerLink="/inicio/cliente/mis-turnos" class="acceso">
        <mat-icon>event</mat-icon><span>Mis turnos</span>
      </a>
      <a mat-card routerLink="/inicio/cliente/mi-perfil" class="acceso">
